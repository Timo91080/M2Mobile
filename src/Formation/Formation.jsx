...
const Formation = () => {
  ...
  useEffect(() => {
    ...
    // Exemple avec fetch :
    fetch('http://10.193.112.177:8082/formations')
      .then((res) => {
        if (!res.ok) {
  ...
  const handleInscription = async (formationId) => {
    try {
      const response = await fetch('http://192.168.1.33:8082/inscription', {
        method: 'POST',
        headers: {
  ...
  const handleFavoris = async (formationId) => {
    try {
      const response = await fetch('http://192.168.1.33:8088/favoris', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
